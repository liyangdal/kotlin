...
public class DuplicateJvmClassNameAnnotator implements Annotator {
    @Override
    public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
        if (!(element instanceof JetPackageDirective) && !(element instanceof JetClassOrObject)) return;
        if (!ProjectRootsUtil.isInProjectSource(element)) return;

        if (element instanceof JetPackageDirective) {
            element = ((JetPackageDirective) element).getContainingJetFile();
        }
